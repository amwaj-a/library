...
const check=()=>{
    ...
    axios.get('https://6689ba9f0ea28ca88b88bd30.mockapi.io/book')
    .then(res=>{
...
{/* <Nav login={true} title='login'/> */}
 ...
 <div id='login' className='w-[50%] 
justify-center max-sm:w-full text-4xl
  gap-4 flex flex-col 
  items-center h-full
   font-sans  relative'>
   
   <Link to={'/'}    className='absolute top-1 mb-2  
    text-xl max-sm:text-lg p-2 '>
      <img className='w-20' src={logo} alt="" />
      </Link>

    <h1 className='text-4xl 
    text-center font-extrabold'>Welcome back! </h1>
  
  <p className='text-center opacity-70 text-lg '>Hello agin </p>

<label  className="bg-gray-300  w-[60%]
  ...
  <input onChange={(e)=>{setapi({...api,'email':e.target.value})}} type="email"    className="
...
<label  className="bg-gray-300
  ...
  onChange={(e)=>{setapi({...api,'password'
...
<button onClick={check} className='bg-40
 text-gray-50 w-[60%] px-2 p-1 rounded-md'>
    Login</button>
    <span className='text-red-500 text-xl'>{texterror}</span>
<span>Don't have an account yet? <Link className='px-2 cursor-pointer text-20 font-bold' to='/signUp'>Sign Up.
</Link> </span>
</div>
 </section>
