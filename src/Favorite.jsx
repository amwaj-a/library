...
function Favorite() {
  const navigat=useNavigate()
    ...
    const [Start, setStar] = React.useState(false);
    React.useEffect(() => {
          //  apiBOOK()
get()
    }, [])
...
<br />
{api.map(r=>console.log(r.book.title))}
...
<section className='grid grid-cols-3 max-md:grid-cols-1  gap-9 w-[90%] m-auto'>
  {api.map(e=>(
    ...
    <div className=' w-[90%]  flex justify-start items-center'> 
     ...
     <img  className='w-60 h-72 shadow-xl shadow-slate-500'
      src={e.book.book_image} alt="" />
...
<div className='flex flex-col items-center gap-3 h-20'>
      ...
      }} className='bg-40  w-40 mt-7 text-gray-50 cursor-pointer px-2 p-1 rounded'>Details</button>
  ...
  }} className=' px-1' >Delete Favorite</span>
...
<h1 className='ms-20 w-max font-extrabold text-xl mt-9 
mb-3 text-40 text-center'>{api.title}</h1>
{/* <p className='pb-4 border-b-[1px] border-gray-700 text-lg text-justify w-96'>{api.description}</p> */}
